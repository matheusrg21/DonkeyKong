...
.text
		...
		la a2,fundo			# endere�o dos dados da tela na memoria
		addi a2,a2,8			# primeiro pixels depois das informa��es de nlin ncol
		jal fundoBMP
		
		li a0,0xFF010F40 		# endereco inicial da Memoria VGA
		li a1,0xFF01220F		# endereco final 
		la a2,jmpd			# endere�o dos dados da tela na memoria
		addi a2,a2,8			# primeiro pixels depois das informa��es de nlin ncol
		li a3 16
		jal boneco
...
CONTA:      	addi s0,s0,1	        	# incrementa o contador
	    	...
	    	j CONTA				# volta ao loop
                                    		...
                                    		### Apenas verifica se h� tecla apertada
...
fundoBMP:	beq a0,a1,SAI			# Se for o �ltimo endere�o ent�o sai do loop
		lb t0,0(a2)			# le um conjunto de 4 pixels : word
		sb t0,0(a0)			# escreve a word na mem�ria VGA
		addi a0,a0,1			# soma 4 ao endere�o
		addi a2,a2,1
...
boneco:		beq a0,a1,SAI
		...
		sb t0,0(a0)
		addi a3 a3 -1
		addi a0,a0,1			# soma 1 ao endere�o
		addi a2,a2,1
